...
SYMBOL = "ETH/USDT"  # MEXC spot/futures symbol format  # инструмент
LEVERAGE = 500  # плечо (фиксированное)
...
MIN_RANDOM_TRADE_SECONDS = 360  # минимальная случайная длительность сделки 6 минут
MAX_RANDOM_TRADE_SECONDS = 720  # максимальная случайная длительность сделки 12 минут
PAUSE_BETWEEN_TRADES = 0  # пауза между сделками убрана
START_BANK = 1000.0  # стартовый банк (для бумажной торговли / учета)
DASHBOARD_MAX = 20
...
class TradingBot:
    ...
    def save_state_to_file(self):
        try:
            with open("goldantilopabtc500_state.json", "w") as f:
    ...
    def load_state_from_file(self):
        try:
            with open("goldantilopabtc500_state.json", "r") as f:
                data = json.load(f)
                state.update(data)
        except:
            pass
    ...
    def place_market_order(self, side: str, amount_base: float):
        ...
        if RUN_IN_PAPER or API_KEY == "" or API_SECRET == "":
            ...
            # Генерируем случайное время закрытия от 6 до 12 минут
    ...
    def strategy_loop(self, should_continue=lambda: True):
        ...
        while should_continue():
            try:
                ...
                dir_5m = dirs["5m"]
                dir_15m = dirs["15m"]
                logging.info(f"[{self.now()}] dirs => 1m:{dir_1m} 5m:{dir_5m} 15m:{dir_15m}")
                ...
                if state["in_position"]:
                    ...
                    # Принудительное закрытие по случайному времени (6-12 минут)
                    ...
                    # Проверка стоп лосс на 40% от ставки (маржи)
                    current_price = self.get_current_price()
                    entry_price = state["position"]["entry_price"]
                    size = state["position"]["size_base"]
                    margin = state["position"].get("margin", abs(state["position"]["notional"]) / 500)
                    ...
                    # Стоп лосс на 40% от ставки (маржи)
                    stop_loss_threshold = -margin * 0.4  # 40% от маржи в минус
                    if current_pnl <= stop_loss_threshold:
                        logging.info(f"❌ СТОП ЛОСС! PnL=${current_pnl:.2f} <= -40% от ставки (${stop_loss_threshold:.2f})")
                        self.close_position(close_reason="stop_loss")
                        state["skip_next_signal"] = True  # устанавливаем флаг пропуска
                        self.save_state_to_file()
                        time.sleep(1)
                        continue
                ...
                # Если не в позиции — проверяем условие входа: SAR 15m и 1m совпадают
                    ...
                    if dir_1m in ["long", "short"] and dir_1m == dir_15m and not state["skip_next_signal"]:
                        ...
                        logging.info(f"Signal to OPEN {side} — 15m:{dir_15m} 1m:{dir_1m} size_base={size_base:.6f} notional=${notional:.2f} price={price}")
