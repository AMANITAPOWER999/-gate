...
⏱️ <b>Entry Time:</b> {datetime.fromisoformat(position["entry_time"]).strftime("%H:%M:%S")}
    ...
    def send_position_closed(self, trade, trade_number=1, balance=0, symbol="TOP1"):
        ...
        message = f"""
<b>{pnl_emoji} TRADE #{trade_number} CLOSED</b>
<b>🎯 {side_text} {symbol_display}/USDT</b> (x10 leverage)

📍 <b>Entry Price:</b> ${trade["entry_price"]:.4f}
🚪 <b>Exit Price:</b> ${trade["exit_price"]:.4f}
📈 <b>Size:</b> {trade["size_base"]:.2f} {symbol_display}
💰 <b>Notional:</b> ${trade["notional"]:.2f}
🔐 <b>Margin:</b> ${margin:.2f}

{profit_emoji} <b>P&L:</b> {pnl_sign}{trade["pnl"]:.2f} USDT
📊 <b>ROI:</b> {pnl_sign}{roi:.2f}%
💳 <b>Balance After Trade:</b> ${balance:.2f}

⌛ <b>Duration:</b> {trade.get("duration", "N/A")}
📌 <b>Close Reason:</b> {close_reason_display}
⏱️ <b>Closed:</b> {datetime.fromisoformat(trade["time"]).strftime("%H:%M:%S")}
        """.strip()
