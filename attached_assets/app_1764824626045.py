...
active_sessions = set()  # Track active user sessions
strategy_config = {
    'open_levels': ['5m', '30m'],
    'close_levels': ['5m']
}
...
def api_login():
    ...
    try:
        ...
        if not referral_verified:
            return jsonify({'error': 'Must verify referral registration first'}), 403
        ...
        api_connected_global = True  # Set global API status flag
        logging.info(f"User authenticated with Gate.io API (UID: {uid}, Balance: ${real_balance:.2f}), api_connected_global=TRUE")
        return jsonify({
...
def api_online_users():
    ...
    return jsonify({'online_users': len(active_sessions)})
...
def api_status():
    ...
    try:
        ...
        # Balance: if GLOBAL API status is TRUE, fetch real balance; otherwise show virtual
        ...
        if api_connected_global:
                    ...
                    if is_valid and isinstance(balance_result, dict):
                        if 'USDT' in balance_result:
                            ...
                            if isinstance(usdt_balance, dict):
                                ...
                                logging.info(f"✅ Fetched REAL balance from Gate.io: ${display_balance:.2f}")
                            else:
            ...
            # API DISCONNECTED: show virtual balance from state (can be negative)
        ...
        return jsonify({
            ...
            'available': round(max(0.0, float(state.get('available', 100.0))), 2),
            ...
            'api_connected': api_connected_global
        })
    except Exception as e:
...
def api_set_strategy_config():
    ...
    try:
        data = request.get_json()
        strategy_config['open_levels'] = data.get('open_levels', [])
        strategy_config['close_levels'] = data.get('close_levels', [])
        logging.info(f"Strategy config updated: open={strategy_config['open_levels']}, close={strategy_config['close_levels']}")
...
def fetch_top_gainers_background():
    ...
    try:
        ...
        # БЛОК: если есть открытая позиция, держи текущую монету на #1 (заблокирована)
        if state.get('in_position', False) and current_trading_symbol:
            for i, gainer in enumerate(gainers):
                if gainer['symbol'] == current_trading_symbol:
                    # Переместить текущую монету на первое место
                    current_gainer = gainers.pop(i)
                    gainers.insert(0, current_gainer)
                    logging.info(f"🔒 БЛОКИРОВКА: {current_trading_symbol} на позиции #1 (в сделке)")
                    break
